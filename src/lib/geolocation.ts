...
export async function requestCurrentPosition(): Promise<GeolocationPosition> {
  if (typeof navigator === "undefined" || !("geolocation" in navigator)) {
    ...
    console.error("[Onlooker LLC location] Geolocation API unavailable", { code: failure.code });
    throw failure;
  }
  ...
  if (navigator.permissions?.query) {
    try {
      ...
      console.info("[Onlooker LLC location] Permission state", { state: permission.state });
      if (permission.state === "denied") {
        const failure = new GeolocationFailure(
          "denied",
          "Location permission is blocked. Enable it in your device settings and try again.",
        );
        console.error("[Onlooker LLC location] Permission denied", { code: failure.code });
        throw failure;
      }
    } catch (error) {
      ...
      console.warn("[Onlooker LLC location] Permission state could not be read; requesting GPS directly");
    }
  }
  ...
  try {
    ...
    console.warn("[Onlooker LLC location] Precise GPS attempt failed", {
      code,
  ...
  try {
    ...
    const positionError = error as GeolocationPositionError;
    const code = positionErrorCode(positionError);
    console.error("[Onlooker LLC location] GPS request failed", {
      code,
      numericCode: positionError.code,
    });
    throw new GeolocationFailure(
