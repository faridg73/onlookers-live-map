...
import { escrowLabel, planLimit, proVisitControls, proVisitView, remainingVisits } from "./pro-dashboard";
...
describe("Pro Dashboard helpers",()=>{
  ...
  it("shows only safe contextual controls",()=>{ expect(proVisitControls(row())).toEqual({canContinue:true,canCancel:false,canReview:false}); expect(proVisitControls(row({bookingStatus:"published",requestId:"r",requestStatus:"open",submissionCount:0}))).toEqual({canContinue:false,canCancel:true,canReview:false}); expect(proVisitControls(row({bookingStatus:"published",requestId:"r",requestStatus:"claimed",submissionCount:1}))).toEqual({canContinue:false,canCancel:false,canReview:true}); });
});